...
// ---------------------------------------------------------------------------
function getBuiltinProviderItems(ctx, cfg) {
...
function getAdditionalProviderItems(cfg) {
    const items = [];
    for (const [key, accounts] of Object.entries(cfg.providers)) {
        if (!key.startsWith("custom:"))
            continue;
...
// ---------------------------------------------------------------------------
async function handleSwitchCommand(ctx, pi) {
    const self = {
        ctx,
        pi,
        ...
        async showProviderSelector(items) {
        ...
        async showAccountSelector(key, accounts, items) {
            const accountId = await promptAccount(this.ctx, key, accounts);
            if (!accountId)
                return this.showProviderSelector(items);
            const account = accounts.find((a) => a.id === accountId);
            if (account)
                await activateAccount(this.ctx, this.pi, key, account);
        },
    };
...
export default function (pi) {
    pi.registerCommand("switch", {
        ...
        getArgumentCompletions: (prefix) => SWITCH_SUBCOMMANDS.filter((v) => v.startsWith(prefix.trim())).map((v) => ({ value: v, label: v })),
        handler: async (args, ctx) => {
            if (ctx.mode !== "tui") {
                ctx.ui.notify("/switch requires interactive TUI mode", "error");
                return;
            }
            const sub = args?.trim();
            if (sub === "models") {
